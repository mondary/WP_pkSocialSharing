...
= 2026.08.09 =
* Ajout d'un lien Dry-run par reseau pour verifier la configuration et le texte genere sans publier.

= 2026.08.08 =
* Effacement des erreurs X historiques apres une publication reussie via le runner navigateur.
* Ajout d'avertissements dans l'interface de langue quand aucun provider n'est actif ou quand la cible correspond a la langue source detectee.

= 2026.08.07 =
* Runner Medium: apres 3 echecs consecutifs, l'article est retire de la queue WP (recuperable via "Remettre dans la queue") pour ne pas marteler Medium.
* Endpoint REST /medium-browser/skip: marque l'article comme ignore et libere le claim.

= 2026.08.06 =
* Runner Medium: confirmation de publication adaptee a la nouvelle page /submission de Medium (boutons Publish/Submit sans seuil de largeur), attente de l'URL finale hors /submission, retrait du contournement "bouton Import" (inutile: le champ URL est present au chargement).
...
= 1.3.12 =
* Controleur macOS: controles de queue Medium (voir, traiter, vider et logs) equivalents a X.

= 1.3.11 =
* Runner X: une publication n'est plus marquee reussie sans confirmation explicite de X; endpoint de reinitialisation du compteur quotidien.

= 1.3.10 =
* Controleur macOS: vue de queue X defilante, titres decodees et claims expires non affiches comme actifs.

= 1.3.9 =
* Controleur macOS: consultation et purge confirmee de la queue X.

= 1.3.8 =
* Bouton X adapte au runner navigateur: remet l'article dans sa queue au lieu de forcer l'API X.
...
= 1.1.8 =
* Partage X via navigateur: evite le doublon d'URL quand l'article est deja inclus dans le texte

= 1.1.7 =
* Partage X via navigateur: ajout de l'URL de l'article dans l'intent pour restaurer l'unfurl Open Graph

= 1.1.6 =
* Renommage du menu admin en WP PK SocialSharing

= 1.1.5 =
* Ajout d'un fallback Open Graph front-end pour restaurer les apercus LinkedIn sur les articles autorises
